...
const mocks = vi.hoisted(() => ({
  ...
  detailOpen: undefined as ReturnType<typeof ref> | undefined,
}));

vi.mock("~/composables/main/usePracticeDetail", () => ({
  usePracticeDetail: () => ({
    detailOpen: mocks.detailOpen,
    closeDetail: () => {
      if (mocks.detailOpen) mocks.detailOpen.value = false;
    },
  }),
}));
...
describe("practice workspace detail panel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.detailOpen = ref(false);
  });

  it("starts centered with the detail panel closed", async () => {
  ...
  it("keeps the related-word rail immediately to the left of practice", async () => {
    ...
    const tool = wrapper.get('[data-test="tool"]');
    const rail = wrapper.get('[data-test="practice-word-rail"]');
    const detail = wrapper.get('[data-test="detail-panel"]');

    expect(workspace.element.firstElementChild).toBe(track.element);
    ...
    expect(pane.element.firstElementChild).toBe(tool.element);
  });
  ...
  it("keeps the game mode and shortcuts in the original flex height flow", async () => {
    ...
    expect(wrapper.get('[data-test="tool"]').element.nextElementSibling).toBe(gameMode.element);
  ...
  it("keeps the current centered width while reserving a collapsible detail track", () => {
    ...
    expect(workspaceSource).not.toContain("workspace-toolbar");
  });
  ...
  it("constrains the related-word rail at regular desktop widths so it scrolls locally", () => {
    ...
    expect(baseStyles).toMatch(
      /\.practice-word-column\s*\{[\s\S]*?height: 100%;[\s\S]*?max-height: 100%;/,
    );
  });
  ...
  it("keeps the lesson rail fixed and splits the remaining desktop width evenly", () => {
    ...
    expect(workspaceSource).toMatch(
      /@media \(min-width: 1280px\)[\s\S]*?\.is-detail-open \.detail-panel\s*\{[\s\S]*?position: absolute;[\s\S]*?top: 0;[\s\S]*?right: 1rem;[\s\S]*?width: calc\(50% - 8rem\);[\s\S]*?height: 100%;[\s\S]*?max-height: 100%;/,
    );
  });
  ...
  it("opens the detail panel from the shared top navigation state", async () => {
    mocks.detailOpen = ref(true);
    const wrapper = await mountPage();
